...
- Runs a 2048-point FFT and publishes log-spaced band energies plus a spectral-
  flux **onset** envelope. **The transform runs on the sender thread, not the
  audio thread** — `Work()` only copies mono samples into a ring buffer, so all
  spectral analysis costs the audio thread nothing.
...
| `/rebuzz/tap/<name>/onset` | transient/onset envelope, spikes to 1 on a hit then decays |
...
- **Bands** (0–8, default 8) — how many FFT bands to publish. `0` disables the
  transform entirely.
- **Onset** (0–127, default 64) — transient detection sensitivity via spectral
  flux; `0` off, higher fires more readily. Emits `/rebuzz/tap/<name>/onset` as a
  0..1 envelope that jumps to 1 on a hit and decays, so it drives a decaying
  flash and can't be missed between frames.
